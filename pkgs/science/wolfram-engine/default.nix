{ stdenv, fakeroot, fetchurl, coreutils, patchelf, requireFile, callPackage
, makeWrapper, alsaLib, dbus, fontconfig, freetype, gcc, glib, libssh2, ncurses
, opencv2, openssl, unixODBC, xkeyboard_config, xorg, zlib, libxml2, libuuid
, lang ? "en", libGL, libGLU }:
...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    fakeroot ./MathInstaller -auto -createdir=y -execdir=$out/bin -targetdir=$out/libexec/Mathematica -verbose
    ...
    wrapProgram $out/bin/WolframKernel --set USE_WOLFRAM_LD_LIBRARY_PATH 1
  '';
